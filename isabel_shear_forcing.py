...
c.retrieve(
    'reanalysis-era5-pressure-levels',
    {
        ...
        'area': [53, -86, 8.4, -26.4],  # N, W, S, E as observed from tracks data, selected the max with buffer

        'format': 'netcdf',
    },
    'isabel_vws_evolution.nc'
)
...
# define vws_mag
ds = nc.Dataset('isabel_vws_evolution.nc') 
...
# STILL DOWNLOADING
ds = nc.Dataset('isabel_surface_V_evolution.nc')  # 28 data sets, contains u10 and v10, 2023-09-06 to 2023-09-19, 00:00 and 12:00
...
for dt in [0, 6, 12, 18, 24, 48]:  # hours
    ...
    # vws = vws_detrended[:29 - steps] if steps > 0 else vws_detrended     # shear at time T (only taking first 20 timestamps for vws to match vmax)
    ...
    vws = vws_diff[:28 - steps] if steps > 0 else vws_diff  # vws_diff has 19 values only
    ...
    plt.title("Vmax vs Annulus-mean VWS for Isabel at dt = {}h".format(dt))
    plt.grid()
plt.show()
